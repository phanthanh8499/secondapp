...
class PhotoCamera extends React.PureComponent {
  ...
  takePhoto = async () => {
    ...
    console.log("nhan ve uri", data.uri)
    let exist = await RNFetchBlob.fs.exists(
      `${RNFetchBlob.fs.dirs.DCIMDir}/myfolder`,
    );

    if (!exist)
      await RNFetchBlob.fs.mkdir(`${RNFetchBlob.fs.dirs.DCIMDir}/myfolder`);
    ...
    if (exist) {
      await RNFetchBlob.fs.mv(
        data.uri,
        `${RNFetchBlob.fs.dirs.DCIMDir}/myfolder/${sec}.jpg`,
      );
    }
    console.log("nhan ve", data)
    console.log("nhan ve base64", data.base64)
